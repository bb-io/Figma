...
public class FileManager : IFileManagementClient
{
    ...
    public async Task DownloadFileAsync(FileReference reference, string? fileUrl = null)
    {
        using (var httpClient = new HttpClient())
        using (var requestMessage = new HttpRequestMessage(HttpMethod.Get, fileUrl ?? reference.Url))
        using (var response = await httpClient.SendAsync(requestMessage))
        {
            response.EnsureSuccessStatusCode();
            var filePath = Path.Combine(outputFolder, reference.Name);
